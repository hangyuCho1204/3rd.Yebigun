...
<body>
	<label><h1>회원정보 입력</h1></label><label style="margin-left: 430px;"><a href="login">login</a></label>
	<hr>
	<form name="form2" class="form-inline" method="post" action="cuInsert">
		<div class="form-group">
			<label for="email">이메일:</label><br>
			<input name="email" class="form-control" type="text" value="asdf@naver.com"><input id="idcheck" class="btn btn-default" type="button" value="중복검사"><span id="check"></span>
		</div><br>
		<div class="form-group">
		<label for="passwd1">비밀번호:</label><br>
			<input name="passwd1" class="form-control" type="password" value="12345"><span>비밀번호는 4~8자리</span>
		</div><br>
		<div class="form-group">
		<label for="passwd2">비밀번호 확인:</label><br>
			<input name="passwd2" class="form-control" type="password" value="12345">
			<input name="passwd" type="hidden">
		</div><br>
		<div class="form-group">
		<label for="name">이름:</label><br>
			      <input name="name" class="form-control" type="text" value="조한규"><span>한글 2~10자리</span>
		</div><br>
		<div class="form-group">
		<label for="phone">휴대폰:</label><br>
			   <input name="phone" class="form-control" type="text" value="010-9527-3376">
		</div><br>
		<div class="form-group">
 ...
 <script type="text/javascript">
		...
		$(function(){
				...
				if(result==true){
					...
					if(!reg.test($("input[name='passwd1']").val())){
						alert("비밀번호를 다시 입력 해주세요.");
						$("input[name='passwd1']").focus();
						result = false;
					}else{
						if($("input[name='passwd1']").val()!=$("input[name='passwd2']").val()){
							alert("비밀번호 확인을 다시 입력 해주세요.");
							$("input[name='passwd2']").focus();
							result = false;
						}else{
							$("input[name='passwd']").attr("value", $("input[name='passwd1']").val());
							result = true;
						}
					}
				}
